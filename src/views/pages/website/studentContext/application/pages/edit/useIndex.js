...
export default () => {
  ...
  const loadApplication = () => {
    return store.dispatch('studentContext/application/showApplicationAsync', { uuid })
      .then(response => {
  ...
  const update = () => {
    store.dispatch('studentContext/application/updateApplicationAsync', { uuid: uuid, data: getData() })
      .then(() => {
        // const { uuid } = response.data
        router.push(Tr.route({ name: 'company-student-show', params: { uuid: studentUuid.value} }))
      })
  }

  const loadStatuses = () => {
    store.dispatch('studentContext/application/loadApplicationStatusesListAsync').then(response => {
