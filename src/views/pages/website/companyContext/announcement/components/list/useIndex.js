...
export default function useIndex() {
  ...
  const items = ref([]);
  const columns = [
    { field: 'author', title: t('companyContext.announcement.form.author'), width: '200px' },
    { field: 'content', title: t('companyContext.announcement.form.content') },
  ...
  const loadItems = (params = {}) => {
    ...
    store.dispatch('companyContext/announcement/loadAnnouncementsAsync', { params: { ...params, ...paginator.toQueryParams(), } })
      .then(response => {
        const { data } = response;
  ...
  const setModalShowData = uuid => {
    store.dispatch('companyContext/announcement/viewAnnouncementAsync', { uuid })
      .then(response => {
