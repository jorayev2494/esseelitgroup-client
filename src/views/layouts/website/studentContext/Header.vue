    ...
    <div class="header-fixed">
      <nav class="navbar navbar-expand-lg header-nav">
        ...
        <ul class="nav header-navbar-rht">
          <ul class="main-nav me-4">
            <LanguageSwitcher />
          </ul>

          <!-- User Menu -->
          <li class="nav-item dropdown has-arrow logged-item" v-if="authData">
            <a
              href="#"
            ...
            <div class="dropdown-menu dropdown-menu-right">
              <div class="user-header">
                ...
                <div class="user-text">
                  <h6>{{ authData.first_name }}</h6>
                  <h6>{{ authData.last_name }}</h6>
                  <!-- <p class="text-muted mb-0">Student</p> -->
                </div>
              </div>
              <!-- <router-link class="dropdown-item" to="/mentor/index">Dashboard</router-link> -->
              <router-link class="dropdown-item" :to="$tMakeRoute({ name: 'student-profile-edit' })">Profile Settings</router-link>
  ...
  setup() {
    ...
    const logout = () => {
      authService.logout().then(() => {
        router.push(Tr.route({ name: 'home' }))
      }).catch(() => {
    ...
    onMounted(() => {
      authData.value = store.getters['studentContext/auth/getAuthData']
